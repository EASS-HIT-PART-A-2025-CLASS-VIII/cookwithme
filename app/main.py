...
# --------------------------
# RECIPES CRUD (Admin write, User read)
# --------------------------
...
def update_recipe_endpoint(
    ...
    updated = update_recipe(session, recipe_id, data)
    if not updated:
        raise HTTPException(status_code=404, detail="Recipe not found")
    return updated
...
def read_all(
    ...
    stats_map = {rid: (float(avg), int(cnt)) for (rid, avg, cnt) in stats}

    out: list[RecipeReadWithStats] = []
    for r in recipes:
...
# --------------------------
# HIGHLIGHTS (Admin write, Public read)
# --------------------------
@app.post("/highlights", response_model=Highlight, status_code=201)
...
def get_reviews(recipe_id: int, session: Session = Depends(get_session)):
    stmt = select(Review).where(Review.recipe_id == recipe_id).order_by(Review.created_at.desc())
...
def get_favorites(
    ...
    fav_ids = session.exec(select(Favorite.recipe_id).where(Favorite.user_id == user.id)).all()
    if not fav_ids:
        return []
    return session.exec(select(Recipe).where(Recipe.id.in_(fav_ids))).all()
